...
def test_default_is_smart_cas():
    ...
    if engine._strict_cas == False:
        ...
        # return False
        raise AssertionError(
            f"Default mode is NOT Smart CAS: strict_cas={engine._strict_cas}"
        )
...
if __name__ == "__main__":
    ...
    results.append(
        ("Smart CAS Mode", run_safe("test_smart_cas_mode", test_smart_cas_mode))
    )
    results.append(
        ("Strict CAS Mode", run_safe("test_strict_cas_mode", test_strict_cas_mode))
    )
    results.append(
        (
            "Default Mode Check",
            run_safe("test_default_is_smart_cas", test_default_is_smart_cas),
        )
    )
