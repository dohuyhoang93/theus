...
try:
    from examples.parallel_demo.safety_workers import (
        worker_read_shm,
...
async def test_serialization_safety(engine):
    ...
    try:
        ...
        await asyncio.to_thread(
            engine.execute_parallel, "worker_serialization", bad_input=sock
        )
        ...
        if (
            "pickle" in err.lower()
            or "attribute" in err.lower()
            or "copy" in err.lower()
