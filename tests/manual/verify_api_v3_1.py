...
def app_logic_process(ctx):
    ...
    print(
        f"   [Process] Mutated local ctx.domain.nested['a'] -> {ctx.domain.nested['a']}"
    )
    ctx.domain.counter += 1
...
async def run_app_developer_api():
    ...
    print(
        f"1. Initial State: {engine.state.domain.counter}, {engine.state.domain.nested}"
    )
    ...
    print(
        f"2. Final State:   {engine.state.domain.counter}, {engine.state.domain.nested}"
    )
...
def run_system_api_safety():
    ...
    try:
        ...
        print(
            "   ⚠️  WARNING: Legacy Deep Mutation SUCCEEDED (As expected for FrozenDict implementation)."
        )
        print(
            f"   Current State Verification: {engine.state.domain['nested']['secret']}"
        )
    except Exception as e:
    ...
    engine.compare_and_swap(
        engine.state.version, {"domain": {"nested": {"secret": "safe"}}}
    )
...
def run_strict_cas_test():
    ...
    print(
        f"   Attempting update 'a' with STALE version {ver} (Current: {current_ver})..."
    )
    try:
