...
class StateMachine:
    ...
    def trigger(self, event: str) -> List[str]:
        ...
        if isinstance(raw_on, bool):
            logger.warning(
                f"⚠️ FSM Warning: 'on' key parsed as Boolean {raw_on}. Use 'events' instead."
            )
            raw_on = {}

        transitions = (
            state_def.get("events") or raw_on or state_def.get("transitions") or {}
        )
        ...
        logger.info(
            f"🔄 FSM Transition: {self.current_state} --[{event}]--> {next_state_name}"
        )
