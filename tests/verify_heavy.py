...
def test_heavy_zone():
    ...
    assert sys_ctx.global_ctx.heavy_tensor == "NEW_HUGE_TENSOR_DATA", (
        "Write to HEAVY failed!"
    )
    ...
    print(
        f"   heavy_tensor: {sys_ctx.global_ctx.heavy_tensor} (Should be 'BAD_TENSOR' - PERSISTED)"
    )
    ...
    if sys_ctx.global_ctx.heavy_tensor == "NEW_HUGE_TENSOR_DATA":
        print(
            "✅ HEAVY Zone rolled back (Shadow Discarded). Optimization verified by code inspection."
        )
    else:
        print(
            f"❌ HEAVY Zone NOT rolled back? Value: {sys_ctx.global_ctx.heavy_tensor}"
        )
        sys.exit(1)
